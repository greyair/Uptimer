...
| Fork `greyair/Uptimer:master` | `10c42ee4afb038ff2b06730b0afde9c12a03ac6a` | Production baseline after monitor-edit compatibility fixes |
| Fork `feature/extended-monitoring` | archived/deprecated | Historical integration/diagnostics branch; do not use as a development or deployment base |

When this file is updated after an upstream sync, update the upstream commit above as well.

The former `feature/extended-monitoring` branch is intentionally not merged wholesale. Its production features were squash-merged into `master`; later D1 read/Low Write fixes were re-applied through dedicated branches, and the monitor-edit compatibility fixes were restored in PR #5. The remaining branch history is primarily integration history, UI experiments, documentation snapshots, and temporary D1 diagnostic commits. New work must branch from current `master`.
...
## Low Write production optimizations

The fork currently runs a Low Write-oriented Cloudflare D1 profile.

Production optimizations include:

- bounded per-monitor heartbeat reads instead of a partitioned full-history window query
- runtime uptime totals reused before falling back to the expensive daily uptime aggregation SQL
- runtime snapshot freshness aligned with monitor cadence
- public snapshot fragment/sharded write paths disabled in the current production profile
- scheduler lease acquisition skipped on idle Cron ticks; due monitors are re-read after lease acquisition to preserve race safety

The legacy sharded/fragment implementation remains in the repository for larger deployments. Do not remove it solely because the current small deployment uses the Low Write path.
